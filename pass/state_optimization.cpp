...
class OptimizationPass : public StateMentPass{
    ...
    OptimizationPass(
        ...
        name_new_var_map_(name_new_var_map_),
        index_(index),
...
StateMent * OptimizationPass::pass_(Block * stat) {
    ...
    return For::make( new Const(0), new Const(1), new Const( circle_num_ ),  block_state ); 
}
StateMent * OptimizationPass::pass_(Gather * stat) {
    const std::string & var_name = stat->node_name_;
    ...
    if( gather_map_it != gather_map_.end() ) {
        ...
        if( gather_info.order_type_ == DisOrder ) {
            ...
            if( mask == 1 ) {
                ...
                gather_state_vec.push_back( LetStat::make( load_index , Load::make( BitCast::make( IncAddr::make( name_var_map_it->second,load_index  ) , type_scalar_ptr2vector_ptr( name_var_map_it->second->get_type() )  ) )));
                ...
                return Gather::make( const_cast<Varience*>(name_varP_varPV_map_it->second),load_index );
            } else {
...
StateMent * OptimizationPass::pass_(Add * stat ) { 
        std::string  node_name = stat->node_name_;
        const auto& reduction_map_it = reduction_map_.find( node_name);
        ...
        if( reduction_map_it != reduction_map_.end() ) {
            ...
            if(reduction_info.order_type_ == IncContinue || reduction_info.order_type_ ==Equel) {
...
StateMent * OptimizationPass::pass_(LetStat * stat) {
    ...
    if( expr_block == NULL) {
        ...
        for(int i = 0 ; i < state_vec->size() ; i++) {
            if(dynamic_cast<LetStat*>( (*state_vec)[i] ) == NULL ) {
...
StateMent * OptimizationPass::pass_(Load * stat) {
    ...
    if( it == name_new_var_map_.end()) {
        ...
        return Block::make( Load::make( BitCast::make( it->second,type_scalar_ptr2vector_ptr(it->second->get_type() ))), 
...
StateMent * OptimizationPass::pass_(Scatter * stat) {
    ...
    const std::string & addr_name = stat->addr_name_;
    const auto & scatter_map_it = scatter_map_.find(addr_name);
...
;
                ...
                if( let_stat != NULL && let_stat->node_name_ == output_name_) {
                    ...
                    if(add_state != NULL ) {
                        ...
                        if(gather_state!=NULL ) {
                            ...
                            outer_init_state_vec.push_back(stat) ;
    ...
    for( auto state : inner_state_vec ) {
        inner_state_vec_new[i] = pass( state );
    }
    ...
    for (auto state : outer_init_state_vec) {
        outer_vec.push_back( pass(state ));
    }
...
StateMent * optimization_state( 
       ...
       state_vec.push_back( LetStat::make(range_num_var, range_num_const));  
       /////////////////////////
       for( const auto & it : same_feature_range_map ) {
            const std::vector<std::pair<int,int>> & index_vec = it.second;
            const int index_vec_num = index_vec.size();
